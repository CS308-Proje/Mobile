...
class MyFriendsLogic {
  ...
  Future<List<Invitation>> fetchUserInvitations() async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        final List<Invitation> userInvitations = (data['invitations'] as List<dynamic>)
            .map((json) {
              final Map<String, dynamic> userJson = json['user_id'] as Map<String, dynamic>;
  ...
  Future<void> updateInvitationStatus(String invitationId, String status) async {
    ...
    // Example:
    final url = Uri.parse('http://10.0.2.2:5001/invitation/update/$invitationId');
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      print('Error updating invitation status. Status code: ${response.statusCode}');
    }
  }
  ...
  Future<void> sendFriendRequest(String userId, String targetUserId) async {
    ...
    final url = Uri.parse('http://10.0.2.2:5001/invitation/createInvitation/$targetUserId');
  ...
  Future<void> allowFriendRecommendations(String friendId) async {
    try {
      ...
      final url = Uri.parse('http://10.0.2.2:5001/friends/allowfriend/$friendId');
      ...
      if (response.statusCode == 200) {
        ...
        print('Error allowing friend recommendations. Status code: ${response.statusCode}');
  ...
  Future<void> disallowFriendRecommendations(String friendId) async {
    try {
      ...
      final url = Uri.parse('http://10.0.2.2:5001/friends/disallowfriend/$friendId');
      ...
      if (response.statusCode == 200) {
        ...
        print('Error disallowing friend recommendations. Status code: ${response.statusCode}');
